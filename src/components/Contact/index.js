...
function ContactPage(){
  ...
  //for handling changes when inputs are clicked off
  function handleChange(evt){
    //if evt target is email then check if the email matchs a regexr
    //if matches do nothing if doesnt, let the user know with error message
    //else check to make sure the message/name has a length if not send error message
    //if no error then it can be submitted
